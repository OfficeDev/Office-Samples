...
function showStatus(message, isError) {
  ...
  //status.innerHTML = "";
  let newInnerHTML =
    `<div class="status-card ms-depth-4 ${isError ? "error-msg" : "success-msg"}">` +
    `<p class="ms-fontSize-24 ms-fontWeight-bold">${isError ? "An error occurred" : ""}</p>` +
    `<p class="ms-fontSize-16 ms-fontWeight-regular">${message}</p>`;
  status.innerHTML = newInnerHTML;
}
